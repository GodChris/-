<?php
	$con=mysqli_connect("127.0.0.1","root","951031");
	mysql_select_db("cars_doctor",$con);
	mysql_query("SET NAMES 'utf8'");
    date_default_timezone_set("PRC");
	$sql="select * from model where status=1;";/////查询表model
	$result=mysql_query($sql);/////mysql_query() 函数执行一条 MySQL 查询;$resul获得表model的内容
	while($row=mysql_fetch_array($result)){/////返回根据从结果集取得的行生成的数组
